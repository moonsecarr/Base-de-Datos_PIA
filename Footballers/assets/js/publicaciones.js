...
document.getElementById('input_CargarMultiCP').addEventListener('change', function (e) {
...
const form = document.getElementById("formsPublicaciones");

form.addEventListener("submit", function(e) {
    e.preventDefault();

    fetch("/crearPublicacion-process", {
        method: "POST",
    ...
    .then(data => {
        ...
        if (data.status === "error") {
            ...
            Swal.fire({
                icon: "success",
                title: "¡Espera  que tu publicacion sea aprobada!",
                text: data.message,
                background: "#0d0c0dff",
                draggable: true,
                customClass: {
...
form.addEventListener("submit", function(e) {
    ...
    fetch("/crearPublicacion-process", { method: "POST", body: new FormData(form) })
  .then(res => {
